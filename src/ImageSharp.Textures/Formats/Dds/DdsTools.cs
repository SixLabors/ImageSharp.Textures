...
internal class DdsTools
{
    ...
    public static long ComputePitch(uint dimension, D3dFormat formatD3D, DxgiFormat formatDxgi, int defaultPitchOrLinearSize = 0)
    {
        ...
        if (formatD3D is D3dFormat.R8G8_B8G8 or D3dFormat.G8R8_G8B8 or D3dFormat.UYVY or D3dFormat.YUY2)
        {
    ...
    internal static int ComputeBCPitch(uint dimension, int bytesPerBlock) => (int)Math.Max(1, (dimension + 3) / 4) * bytesPerBlock;
    ...
    internal static int ComputeUncompressedPitch(uint dimension, int bitsPerPixel) => (int)Math.Max(1, ((dimension * bitsPerPixel) + 7) / 8);
    ...
    public static long ComputeLinearSize(uint width, uint height, D3dFormat formatD3D, DxgiFormat formatDxgi, int defaultPitchOrLinearSize = 0)
    {
        ...
        if (formatD3D is D3dFormat.R8G8_B8G8 or D3dFormat.G8R8_G8B8 or
            D3dFormat.UYVY or D3dFormat.YUY2)
        {
    ...
    internal static long ComputeBCLinearSize(uint width, uint height, int bytesPerBlock) => ComputeBCPitch(width, bytesPerBlock) * Math.Max(1, (height + 3) / 4);
}
