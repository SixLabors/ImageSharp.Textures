...
internal struct KtxHeader
{
    ...
    public static KtxHeader Parse(ReadOnlySpan<byte> data)
    {
        ...
        KtxEndianness endianness = (KtxEndianness)BinaryPrimitives.ReadUInt32LittleEndian(data);
        if (endianness is not KtxEndianness.BigEndian and not KtxEndianness.LittleEndian)
        {
        ...
        if (endianness == KtxEndianness.LittleEndian)
        {
            return new KtxHeader(
                (KtxEndianness)BinaryPrimitives.ReadUInt32LittleEndian(data[..4]),
        ...
        return new KtxHeader(
            (KtxEndianness)BinaryPrimitives.ReadUInt32BigEndian(data[..4]),
