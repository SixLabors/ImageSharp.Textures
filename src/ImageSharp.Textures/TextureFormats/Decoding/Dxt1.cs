...
internal struct Dxt1 : IBlock<Dxt1>
{
    ...
    /// <inheritdoc/>
    public readonly byte[] Decompress(byte[] blockData, int width, int height)
    {
        Dxt1 self = this;
        ImageSharp.PixelFormats.Rgb24[] colors = new ImageSharp.PixelFormats.Rgb24[4];
        ...
        return Helper.InMemoryDecode<Dxt1>(blockData, width, height, (stream, data, streamIndex, dataIndex, stride) =>
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                // Every 2 bit is a code [0-3] and represent what color the current pixel is.
