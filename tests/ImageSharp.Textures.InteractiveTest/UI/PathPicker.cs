...
namespace Phoenix.Import.Application.UI
{
    public class PathPicker
    {
        ...
        private static IEnumerable<string> GetSpecialFolders()
        {
            ...
            {
                specialFolders.Add($"/|/");
                specialFolders.Add($"User|{System.Environment.GetFolderPath(System.Environment.SpecialFolder.UserProfile)}");
                specialFolders.Add($"Desktop|{System.Environment.GetFolderPath(System.Environment.SpecialFolder.DesktopDirectory)}");
                specialFolders.Add($"Documents|{System.Environment.GetFolderPath(System.Environment.SpecialFolder.MyDocuments)}/Documents");
            }
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                specialFolders.Add($"User|{System.Environment.GetFolderPath(System.Environment.SpecialFolder.UserProfile)}");
                specialFolders.Add($"Desktop|{System.Environment.GetFolderPath(System.Environment.SpecialFolder.DesktopDirectory)}");
                specialFolders.Add($"Documents|{System.Environment.GetFolderPath(System.Environment.SpecialFolder.MyDocuments)}");
            }
        ...
        {
            var fileEntries = Directory.EnumerateFileSystemEntries(path).OrderBy(f => f);
        ...
        public bool Render()
        {
            ...
            ImGui.InputText("###file-path", ref _selectedFolder, 300, ImGuiInputTextFlags.None);
            ImGui.PopItemWidth();
            ImGui.Spacing();
            ...
            if (ImGui.BeginChildFrame(2, new Vector2(size.X - 224, size.Y - 100), ImGuiWindowFlags.None))
            {
                ...
                {
                    Debug.Print($"Unable to process path '{_selectedFolder}'.");
                }
                ImGui.EndChildFrame();
            }
