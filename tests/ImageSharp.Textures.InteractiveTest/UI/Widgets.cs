...
public static class Widgets
{
    public static void RenderSpinner(Vector2 position, float radius, int thickness)
    {
        ...
        Vector2 centre = new Vector2(position.X + radius, position.Y + radius + ImGui.GetStyle().FramePadding.Y);

        drawList.PathClear();
        for (int i = 0; i < num_segments; i++)
        {
            float a = aMin + (i / (float)num_segments * (aMax - aMin));
            Vector2 location = new Vector2(centre.X + (MathF.Cos(a + (time * 8)) * radius), centre.Y + (MathF.Sin(a + (time * 8)) * radius));
