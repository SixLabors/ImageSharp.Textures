...
public class TestTextureProvider : ITestTextureProvider
{
    ...
    public void SaveTextures(Texture texture)
    {
        ...
        {
            this.SaveMipMaps([.. cubemapTexture.PositiveX.MipMaps], "positive-x");
            this.SaveMipMaps([.. cubemapTexture.NegativeX.MipMaps], "negative-x");
            this.SaveMipMaps([.. cubemapTexture.PositiveY.MipMaps], "positive-y");
            this.SaveMipMaps([.. cubemapTexture.NegativeY.MipMaps], "negative-y");
            this.SaveMipMaps([.. cubemapTexture.PositiveZ.MipMaps], "positive-z");
            this.SaveMipMaps([.. cubemapTexture.NegativeZ.MipMaps], "negative-z");
        }

        if (texture is FlatTexture flatTexture)
        {
            this.SaveMipMaps([.. flatTexture.MipMaps], null);
        }
        ...
        if (texture is VolumeTexture volumeTexture)
        {
            ...
            {
                this.SaveMipMaps([.. volumeTexture.Slices[i].MipMaps], string.Format(CultureInfo.InvariantCulture, "slice{0}", i + 1));
            }
        }
    }
