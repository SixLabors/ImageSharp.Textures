...
public class TolerantImageComparer : ImageComparer
{
    ...
    public override ImageSimilarityReport<TPixelA, TPixelB> CompareImages<TPixelA, TPixelB>(Image<TPixelA> expected, Image<TPixelB> actual)
    {
        ...
        Vector4[] aBuffer = new Vector4[width];
        Vector4[] bBuffer = new Vector4[width];

        float totalDifference = 0F;

        List<PixelDifference> differences = new();
        ImageSharp.Configuration configuration = new();
    ...
    private static float GetManhattanDistanceInRgbaSpace(ref Vector4 a, ref Vector4 b) => Diff(a.X, b.X) + Diff(a.Y, b.Y) + Diff(a.Z, b.Z) + Diff(a.W, b.W);
