...
public class Ktx2ProcessorTests
{
    ...
    {
        Ktx2Processor processor = new(MakeCubemapHeader());
        LevelIndex[] levelIndices = [MakeLevelIndex(0, uncompressedByteLength, uncompressedByteLength)];

        using MemoryStream stream = new(new byte[(int)uncompressedByteLength]);
    ...
    public void DecodeMipMaps_TruncatedStream_Throws()
    {
        Ktx2Header header = new(
            ...
            sgdByteOffset: 0,
            sgdByteLength: 0);
        Ktx2Processor processor = new(header);
